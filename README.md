<div align="center">

# Vitor Lemes

### Eng. de Software (em formação) — Python · JavaScript/Node · Foco em IA/ML

<img src="https://skillicons.dev/icons?i=python,js,nodejs,html,css,vercel,git,github&theme=dark" alt="stack icons" />

<br/>

[![Instagram](https://img.shields.io/badge/Instagram-%40vitor_.rl056-E4405F?style=for-the-badge&logo=instagram&logoColor=white)]
![LinkedIn](https://img.shields.io/badge/LinkedIn-em%20breve-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)

</div>

<br/>

<div align="center">

**🇧🇷 Português abaixo** · scroll down or click below for **🇺🇸 English**

</div>

---

## Sobre

- Estudo Python há um bom tempo — hoje construo programas de organização de arquivos, simulações 3D e experimentos com IA/prompts.
- Trabalho com JavaScript e Node.js em projetos particulares; uso HTML/CSS no front-end.
- Aprendendo deploy e infraestrutura web com Vercel.
- Curioso por segurança ofensiva — estudo em ambientes controlados (VM), por interesse pessoal. Não é foco de carreira, é hobby.
- Objetivo de carreira: Engenharia de Software, com interesse crescente em IA/ML e LLMs.

## Formação em andamento

| Curso | Instituição |
|---|---|
| Python | Fundação Bradesco |
| Empreendedorismo | SEBRAE |

## Stack

**Domínio / uso constante**

<img src="https://skillicons.dev/icons?i=python,js,nodejs,html,css&theme=dark" alt="core stack" />

**Aprendendo agora**

![Vercel](https://img.shields.io/badge/Vercel-Deploy-000000?style=flat-square&logo=vercel&logoColor=white)
![AI](https://img.shields.io/badge/IA%2FML-explorando-6E56CF?style=flat-square&logo=openai&logoColor=white)
![Prompt Engineering](https://img.shields.io/badge/Prompt%20Engineering-praticando-6E56CF?style=flat-square)

**Hobby técnico**

<img src="https://skillicons.dev/icons?i=cpp&theme=dark" alt="cpp" height="40"/>
![Offensive Security](https://img.shields.io/badge/Offensive%20Security-VM%20only-2E2E2E?style=flat-square&logo=linux&logoColor=white)

## Projetos

<!--START_SECTION:repos-->
> Nenhum repositório público encontrado ainda.

<!--END_SECTION:repos-->

## GitHub Stats

<div align="center">

<img src="https://github-stats-extended.vercel.app/api?username=VRx-xDev&show_icons=true&theme=tokyonight&hide_border=true&border_radius=12&count_private=true" alt="GitHub Stats" height="165"/>
<img src="https://github-stats-extended.vercel.app/api/top-langs/?username=VRx-xDev&layout=compact&theme=tokyonight&hide_border=true&border_radius=12" alt="Top Languages" height="165"/>

<img src="https://streak-stats.demolab.com/?user=VRx-xDev&theme=tokyonight&hide_border=true&border_radius=12" alt="GitHub Streak" />

<img src="https://github-readme-activity-graph.vercel.app/graph?username=VRx-xDev&theme=tokyo-night&hide_border=true&border_radius=12&area=true" alt="Activity Graph" width="95%"/>

<!--START_SECTION:snake-->
<img src="https://raw.githubusercontent.com/VRx-xDev/VRx-xDev/output/github-contribution-grid-snake-dark.svg" alt="Contribution Snake" width="95%"/>
<!--END_SECTION:snake-->

</div>

> Os cards acima podem demorar a preencher até você ter commits/repositórios públicos. A cobrinha (`snake`) só aparece depois de configurar o GitHub Action abaixo.

---

<details>
<summary><b>🇺🇸 Click to read in English</b></summary>

<br/>

## About

- Studying Python for a while now — currently building file organization tools, 3D simulations, and AI/prompt experiments.
- Working with JavaScript and Node.js on personal projects; using HTML/CSS on the front end when needed.
- Learning web deployment with Vercel.
- Curious about offensive security — practicing in controlled VM environments, purely as a personal interest. Not a career focus, just a technical hobby.
- Career goal: Software Engineering, with growing interest in AI/ML and LLMs.

## Ongoing Courses

| Course | Institution |
|---|---|
| Python | Fundação Bradesco |
| Entrepreneurship | SEBRAE |

## Stack

**Core / daily use:** Python, JavaScript, Node.js, HTML, CSS

**Currently learning:** Vercel deployment, AI/ML fundamentals, prompt engineering

**Technical hobby:** Offensive security (VM environments only)

## Projects

Repositories auto-listed below once published — see the main section above (the `<!--START_SECTION:repos-->` block updates automatically).
> Nenhum repositório público encontrado ainda.

